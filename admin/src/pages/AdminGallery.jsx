...
const AdminGallery = () => {
  ...
  useEffect(() => {
    const fetchAlbums = async () => {
      try {
        const response = await axios.get('http://localhost:5000/api/albums/album-getall');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isEditing) {
        const response = await axios.patch(`http://localhost:5000/api/albums/album/${editId}`, data, {
        ...
        const response = await axios.post('http://localhost:5000/api/albums/album-post', data, {
          headers: { 'Content-Type': 'multipart/form-data' },
        });
  ...
  const handleDelete = async (_id) => {
    ...
    try {
      await axios.delete(`http://localhost:5000/api/albums/album/${_id}`);
